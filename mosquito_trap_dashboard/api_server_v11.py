"""
Mosquito Trap Dashboard — API Server V11
==========================================
Full-featured backend with 30+ endpoints, L1 in-memory cache, materialized views.
Port: 7807 (configurable via MOSQ_PORT env var).

Usage:
    python api_server_v11.py
    MOSQ_PORT=8000 MOSQ_DEBUG=1 python api_server_v11.py
"""
...
@app.route('/')
def serve_dashboard():
    return send_from_directory(project_path, 'dashboard_v11.html')
...
@app.route('/api/v11/kpi', methods=['GET'])
@cached(ttl=60, prefix='v11_kpi')
def v11_kpi():
    where_sql, values = build_where()
    sql = f"""
        SELECT
...
@app.route('/api/v11/table', methods=['GET'])
@cached(ttl=30, prefix='v11_table')
def v11_table():
    where_sql, values = build_where()
    page = int(request.args.get('page', 1))
...
@app.route('/api/v11/species/breakdown', methods=['GET'])
@cached(ttl=60, prefix='v11_sp_breakdown')
def v11_species_breakdown():
    where_sql, values = build_where()
    sql = f"""
        SELECT
            detected_name,
...
@app.route('/api/v11/species/by-area', methods=['GET'])
@cached(ttl=60, prefix='v11_sp_byarea')
def v11_species_by_area():
...
@app.route('/api/v11/species/diversity', methods=['GET'])
@cached(ttl=120, prefix='v11_sp_diversity')
def v11_species_diversity():
    where_sql, values = build_where()
...
@app.route('/api/v11/species/genus', methods=['GET'])
@cached(ttl=120, prefix='v11_sp_genus')
def v11_species_genus():
    where_sql, values = build_where()
...
@app.route('/api/v11/species/matrix', methods=['GET'])
@cached(ttl=120, prefix='v11_sp_matrix')
def v11_species_matrix():
...
@app.route('/api/v11/trends/monthly', methods=['GET'])
@cached(ttl=60, prefix='v11_tr_monthly')
def v11_trends_monthly():
    where_sql, values = build_where()
    sql = f"""
        WITH monthly AS (
            SELECT
...
@app.route('/api/v11/trends/daily-counts', methods=['GET'])
@cached(ttl=60, prefix='v11_tr_daily')
def v11_trends_daily_counts():
    where_sql, values = build_where()
    sql = f"""
        SELECT date, COUNT(*) AS count,
               COUNT(DISTINCT detected_name) AS species_count
...
@app.route('/api/v11/trends/seasonal', methods=['GET'])
@cached(ttl=120, prefix='v11_tr_seasonal')
def v11_trends_seasonal():
    where_sql, values = build_where()
    sql = f"""
        SELECT
            CASE
...
@app.route('/api/v11/trends/yoy', methods=['GET'])
@cached(ttl=120, prefix='v11_tr_yoy')
def v11_trends_yoy():
...
@app.route('/api/v11/trends/dow', methods=['GET'])
@cached(ttl=120, prefix='v11_tr_dow')
def v11_trends_dow():
    where_sql, values = build_where()
...
@app.route('/api/v11/trends/anomalies', methods=['GET'])
@cached(ttl=120, prefix='v11_tr_anomalies')
def v11_trends_anomalies():
...
@app.route('/api/v11/geo/heatmap', methods=['GET'])
@cached(ttl=60, prefix='v11_geo_heat')
def v11_geo_heatmap():
    where_sql, values = build_where()
    ...
    sql = f"""
        SELECT
            ...
            COUNT(*) AS intensity,
            detected_name AS species
        FROM {TABLE_NAME} {cond}
        GROUP BY ROUND(CAST(latitude AS REAL), 4), ROUND(CAST(longitude AS REAL), 4)
        ORDER BY intensity DESC
    """
    return query_db(sql, values)


@app.route('/api/v11/geo/density-by-area', methods=['GET'])
@cached(ttl=120, prefix='v11_geo_density')
def v11_geo_density_by_area():
    where_sql, values = build_where()
    sql = f"""
        SELECT city, state,
               COUNT(*) AS count,
...
@app.route('/api/v11/geo/timeseries', methods=['GET'])
@cached(ttl=120, prefix='v11_geo_ts')
def v11_geo_timeseries():
...
@app.route('/api/v11/ai/confidence-dist', methods=['GET'])
@cached(ttl=120, prefix='v11_ai_dist')
def v11_ai_confidence_dist():
    where_sql, values = build_where()
    sql = f"""
        SELECT
            CASE
...
@app.route('/api/v11/ai/model-comparison', methods=['GET'])
@cached(ttl=120, prefix='v11_ai_models')
def v11_ai_model_comparison():
    where_sql, values = build_where()
    sql = f"""
        SELECT
            aimodel,
...
@app.route('/api/v11/ai/low-confidence', methods=['GET'])
@cached(ttl=60, prefix='v11_ai_low')
def v11_ai_low_confidence():
...
@app.route('/api/v11/ai/confidence-by-species', methods=['GET'])
@cached(ttl=120, prefix='v11_ai_sp')
def v11_ai_confidence_by_species():
    where_sql, values = build_where()
    sql = f"""
        SELECT
            detected_name,
...
@app.route('/api/v11/ai/model-timeline', methods=['GET'])
@cached(ttl=120, prefix='v11_ai_timeline')
def v11_ai_model_timeline():
    where_sql, values = build_where()
    sql = f"""
        SELECT
            aimodel,
...
@app.route('/api/v11/ai/corrections', methods=['GET'])
@cached(ttl=120, prefix='v11_ai_corrections')
def v11_ai_corrections():
    where_sql, values = build_where()
    sql = f"""
...
@app.route('/api/v11/risk/scores', methods=['GET'])
@cached(ttl=60, prefix='v11_risk_scores')
def v11_risk_scores():
...
@app.route('/api/v11/risk/spikes', methods=['GET'])
@cached(ttl=60, prefix='v11_risk_spikes')
def v11_risk_spikes():
...
@app.route('/api/v11/risk/vector-species', methods=['GET'])
@cached(ttl=120, prefix='v11_risk_vectors')
def v11_risk_vector_species():
    where_sql, values = build_where()
    vector_list = [
...
@app.route('/api/v11/risk/weather-correlation', methods=['GET'])
@cached(ttl=120, prefix='v11_risk_weather')
def v11_risk_weather_correlation():
    where_sql, values = build_where()
    sql = f"""
        SELECT
            date,
        ...
        FROM {TABLE_NAME}
        {where_sql}
        {"AND" if where_sql else "WHERE"} temperature_c IS NOT NULL
        GROUP BY date
        ORDER BY date
    """
    rows = query_db(sql, values)
...
@app.route('/api/v11/alerts', methods=['GET'])
@cached(ttl=30, prefix='v11_alerts')
def v11_alerts():
...
@app.route('/api/v11/alerts/timeline', methods=['GET'])
@cached(ttl=60, prefix='v11_alerts_tl')
def v11_alerts_timeline():
...
@app.route('/api/v11/filters/areas', methods=['GET'])
@cached(ttl=LONG_TTL, prefix='v11_flt_areas')
def v11_filter_areas():
    sql = f"""
...
@app.route('/api/v11/filters/species', methods=['GET'])
@cached(ttl=LONG_TTL, prefix='v11_flt_species')
def v11_filter_species():
    sql = f"""
...
@app.route('/api/v11/filters/models', methods=['GET'])
@cached(ttl=LONG_TTL, prefix='v11_flt_models')
def v11_filter_models():
    sql = f"""
...
@app.route('/api/v11/meta/refresh', methods=['POST', 'GET'])
def v11_meta_refresh():
...
@app.route('/api/v11/meta/info', methods=['GET'])
def v11_meta_info():
...
if __name__ == '__main__':
    print(f"=" * 60)
    print(f"  Mosquito Trap Dashboard — API Server V11")
    print(f"=" * 60)
    ...
    print(f"")
    print(f"  V11 Endpoints:")
    print(f"    /api/v11/kpi                  - KPI summary")
    print(f"    /api/v11/table                - Paginated data table")
    print(f"    /api/v11/species/*            - 5 species endpoints")
    print(f"    /api/v11/trends/*             - 6 trends endpoints")
    print(f"    /api/v11/geo/*                - 3 geo endpoints")
    print(f"    /api/v11/ai/*                 - 6 AI analysis endpoints")
    print(f"    /api/v11/risk/*               - 4 risk endpoints")
    print(f"    /api/v11/alerts               - Dynamic alerts")
    print(f"    /api/v11/alerts/timeline       - Alert timeline")
    print(f"    /api/v11/filters/*            - 3 filter endpoints")
    print(f"    /api/v11/meta/refresh         - Rebuild aggregations")
    print(f"    /api/v11/meta/info            - Server info")
    print(f"")
